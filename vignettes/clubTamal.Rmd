...
$$(X'X)^{-1} \, \sum_{j=1}^M u'_{M} u_{M} \, (X'X)^{-1} \cdot \frac{M}{M-1} \frac{N-1}{N-K}$$
where $u = X_{j} e_{j}$.

## Clustering Fixed Effects (FE) estimations

$$\frac{(N-K)}{(N-K)-(M-1)} \cdot (X'X)^{-1} \, \sum_{j=1}^M u'_{M} u_{M} \, (X'X)^{-1} \cdot \frac{M}{M-1} \frac{N-1}{N-K}$$
where $u = X_{j} e_{j}$.
...
Elias Cisneros (2017). clubTamal: Clustered covariance matrix for panel
  data estimations. R package version 0.1.2.
